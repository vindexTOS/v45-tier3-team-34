...
export default function Application_drop_down({ data }: { data: any }) {
  ...
  return (
    <div ref={ref} className=" py-4 flex flex-col gap-2 realtive  ">
      <div
        onClick={() => navigate(`/Developer/${dev_id}`)}
        className="flex items-center p-2 md:px-6 gap-2 cursor-pointer"
      >
        <img className="w-[50px] h-[50px] rounded-full object-contain" src={avatar} />
        <h1 className='text-slate-800 dark:text-slate-200 text-xl'>{userName}</h1>
      </div>
      ...
      <div className="flex items-end justify-end px-10">
        <button
          ...
          className="bg-green-600 dark:bg-green-600 hover:dark:bg-slate-900 dark:hover:text-green-500 py-1 pb-2 text-white hover:text-slate-900 dark:text-slate-700  hover:bg-green-500 w-fit px-4 rounded-[40px] shadow-md "
        >
          See application
        </button>
      </div>

    ...
    return (
      <div className=" w-full h-full flex ">
        <div className="px-1 sm:px-4 gap-5 flex flex-col p-4 max-h-[800px] overflow-y-auto bg-white dark:bg-slate-800 rounded-l-2xl">
          <h1
            className='text-lg sm:text-2xl font-thin hidden sm:block'
          >Chats</h1>
          {chatData.length > 0
            ? chatData?.map((val: any) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 shadow-md p-2 rounded-[9px] relative cursor-pointer hover:bg-green-600 hover:dark:bg-slate-300/50 hover:dark:text-slate-800 w-fit sm:w-full
                    ${selectedChat === id
                        ? 'bg-green-500/30 dark:text-slate-200 dark:bg-slate-900'
                  ...
                  >
                    ...
                    <div className='flex gap-2 sm:gap-2 justify-center items-center w-fit   '>
                      ...
                      <h1 className="hidden sm:block text-center font-normal  h-full">{name}</h1>
                    </div>
                  </div>
                )
              })

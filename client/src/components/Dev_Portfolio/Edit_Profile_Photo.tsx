    ...
    try {
      if (isUserLoggedIn) {
        ...
        const res = await axios.patch(
          ...
          }/crud/${UserState.userData.user._id}`,
  ...
  if (isUserLoggedIn) {
    return (
      <div className="w-[100%] h-[100%] rounded-[20px] absolute flex items-center justify-center  backdrop-blur-md  bg-gray-300/10  ">
        <div className="w-[500px] h-[400px] bg-white  rounded-[20px] shadow-md flex items-center justify-center flex-col gap-2">
          ...
          <button
            ...
            className="bg-primary text-white text-[2rem] px-4 py-1 rounded-[12px] hover:bg-primary-hover"
          >
            Change
          </button>
        </div>
